...
if(argv.help) {
    ...
    npm.load(conf, function(err) {
        ...
        step(function() {
            ...
            var getVersionInfo = function(packageName, cb) {
                npm.commands.show([packageName, 'versions'], true, function(err, data) {
                    ...
                    cb(null, {
                        "package": packageName,
                        "specifiedVersion": conf.dependencies[packageName],
                        "latestVersion": versions[versions.length - 1]
                    });
                });
            };
            for(var packageName in conf.dependencies) {
                if(conf.dependencies.hasOwnProperty(packageName)) {
                    getVersionInfo(packageName, group());
                }
            }
